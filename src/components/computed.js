export default {
  ...
  $c_totals () {
    return Object.assign({}, ...this.headerFields.filter(field => field.item.total).map(field => {
      return { [field.item.key]: this.$c_items.reduce((s, item) => s + (field.item.total.parse ? field.item.total.parse(item[field.item.key]) : item[field.item.key]), 0) }
    }))
  },
  // search
  $c_searchableFields () {
  ...
  $c_items () {
    ...
    if (this.sortKey && typeof items[0][this.sortField] !== 'undefined') {
      if (this.sortOrder === 'asc') {
        ...
        if (typeof this.items[0][this.sortField] === 'number') {
    ...
    if (this.models.search) {
      items = items.filter(item => {
        for (let key of this.$c_searchableFields) {
